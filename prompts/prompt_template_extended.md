...
2.  **Update Queries File:** Overwrite the `studies/[STUDY_NAME]/queries.txt` file with the generated **PubMed queries only**. Include the three main strategies (High-recall, Balanced, and High-precision), any "Minimal Revised Queries", and the top-3 precision-lean PubMed variants that preserve recall. Each query must be preceded by a descriptive comment line (starting with '#') and separated by a blank line.
